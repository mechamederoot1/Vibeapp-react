...
const Profile = () => {
    ...
    try {
      ...
      try {
        await postsAPI.createPost({
          ...
          imageUrl: response.data?.profile_cover_url || response.data?.data_url || updatedUser.coverPhoto
        })
      } catch (postError) {
        console.log('Erro ao criar post de atualização:', postError)
      }
  ...
  const handlePostClick = (post) => {
    if (!post) return
    // Profile cover updates: prefer immutable cover URL if available
    if (post.profileUpdateType === 'cover' && post.imageUrl) {
      const m = String(post.imageUrl || '').match(/\/profile\/cover\/id\/([^?&#]+)/)
      if (m && m[1]) {
        navigate(`/profile/cover/id/${m[1]}`)
        return
      }
    }
    if (post.publicId) {
      ...
      navigate(`/posts/id/${post.publicId}`)
      return
    }
